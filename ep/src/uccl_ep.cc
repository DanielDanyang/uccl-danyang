...
class UCCLGinResourceHandle {
 public:
  UCCLGinResourceHandle(nb::iterable proxies, uintptr_t window_base,
    ...
    if (proxy_ptrs.empty()) {
      throw std::invalid_argument("UCCL-GIN resources require at least one proxy");
    }
    ...
    for (auto* proxy : proxy_ptrs) {
      ...
      device_handle_addrs.insert(device_handle_addrs.end(), addrs.begin(), addrs.end());
    }
    ...
    num_lanes_ = static_cast<uint32_t>(num_lanes <= 0 ? proxy_ptrs.size() : num_lanes);
  }
  ...
  nb::dict as_dict() const {
    nb::dict d;
    d["d2h_queues_ptr"] =
        static_cast<unsigned long long>(reinterpret_cast<uintptr_t>(d2h_queues_device_));
...
NB_MODULE(ep, m) {
  ...
  m.def("d2h_queue_capacity",
        []() { return static_cast<std::uint32_t>(kQueueSize); });
