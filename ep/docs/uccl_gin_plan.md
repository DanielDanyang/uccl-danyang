...
| `put_value<team>(ptr,val,dst)` | `st_relaxed_sys` | 需要本地 staging 后 D2H WRITE | **P2 按 call site 补** |
| `red_add_rel<team>(ptr,val,dst)` | `red_add_rel_sys` | compact tail op + PackAtomicWithSeq | **不是透明 drop-in** |
...
ep/                                  ← UCCL 拥有的全部后端 (clean git)
  include/uccl_gin/                  [待建] UCCL-GIN device backend；不要复活 v2_efa fork
    uccl_gin_handle.cuh    [新] namespace deep_ep::elastic::handle { struct UCCLGin }
...
### 4.2 `red_add_rel<Rail>` / tail —— 有序软原子,但不是透明 drop-in

```
Lsa : ptx::red_add_rel_sys(dst, value)                 ← 不变
Rail tail:
      不能直接把 NCCL 的 packed (finish,count) window add 透传给 UCCL:
        1. UCCL proxy 在 CPU 侧 apply,目标必须是 host-mapped atomic buffer,
           不是 DeepEP window/HBM tail word;
        2. ordered-atomic immediate 的 value 只有 15 bit,offset 只有约 13 bit,
           装不下原 packed tail / 大 window offset。

      因此 dispatch tail 用 compact slot:
        slot = channel_idx * num_scaleout_ranks + src_scaleout_rank
        periodic update: add count_delta
        final update   : add 8192 + count_delta
      receiver forward warp 从同一个 compact slot 读 raw:
        finish = raw >= 8192
        count  = raw - (finish ? 8192 : 0)

      所有 add 仍走 PackAtomicWithSeq:
        seq = next_seq_per(dst_rank, slot)++  (mod kReorderingBufferSize)
        D2H ATOMIC cmd (atomic_offset!=0) → proxy
        receiver proxy reorder buffer 按 seq 有序 apply
```
这是 EFA 无硬件 atomic / 无序的"语义补齐",但它需要 kernel 写侧和读侧一起改,
所以放在 `UCCLGin::rail_tail_add()/rail_tail_ptr()/decode_rail_tail()` 显式 API,
而不是假装 `red_add_rel<Rail>(window_tail_ptr, packed_delta)` 能透明工作。
...
put_value<Rail>  : 不能直接从 immediate value 变出本地 source pointer;需要 per-lane
                   staging/scratch 后 D2H WRITE,或按具体 call site 改成 put/atomic。
...
P2  kernel + comm.cuh 接线 (档位 2)                        [打通 dispatch]
      a. thirdparty 极小 patch:hybrid_dispatch.cuh 在 DEEPEP_USE_UCCL_GIN 下构造 UCCLGin
      b. ★ patch comm.cuh:barrier 系列模板化 gin 类型 (default NCCLGin)
         —— 否则 kernel 用 UCCLGin 时连 scaleup barrier 都编不过(见 §4.5)
      c. tail 写/读两侧改成 compact atomic-tail slot(见 §4.2)
      d. Rail barrier 依赖 Phase-1 已删,不需 UCCLGin Rail barrier
